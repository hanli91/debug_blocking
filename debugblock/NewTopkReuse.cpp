...
//    }
    string str = "";
    char buf[10];
    for (int i = 0; i < remained_fields.size(); ++i) {
        sprintf(buf, "%d", remained_fields[i]);
        if (i != 0) {
            str.append(string(" "));
        }
        str.append(buf);
    }
    printf("config: %s  new topk number of compared pairs: %ld\n", str.c_str(), total_compared_pairs);
    printf("config: %s  new topk number of reused pairs: %ld\n", str.c_str(), reuse_count);
}
